...
class ListOfSizeInFilterWidget extends ConsumerWidget {
  const ListOfSizeInFilterWidget({super.key, required this.isSearchFilter,required this.size,required this.idCategory,required this.nameSearch});
        ...
        return InkWell(
          onTap: () {
            if (stateFilter.stateData != States.loading) {
              ref.read(selectedSizesProvider(idCategory).notifier).toggleSize(item.id!);
